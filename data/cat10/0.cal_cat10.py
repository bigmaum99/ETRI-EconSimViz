...
def process_json_data(data):
    ...
    for item in data['data']:
        to_cat10_cd = item['to_cat10_cd']
        from_cat10_cd = item['fr_cat10_cd']
        
        # to_cat10_cd가 "1"로 시작하고 from_cat10_cd도 "1"로 시작하는 데이터 필터링
        if to_cat10_cd.startswith("1") and from_cat10_cd.startswith("1"):
            if to_cat10_cd not in result:
                result[to_cat10_cd] = 0
            result[to_cat10_cd] += item['flow_valueB']
...
def main():
    ...
    for to_cat10_cd, total_flow_valueB in processed_data.items():
        ...
        line1 = f"{to_cat10_cd}>FIN;{total_flow_valueB}"
        result_lines.append(line1)
        
    result_lines.append("")
    for to_cat10_cd, total_flow_valueB in processed_data.items():
        # 두 번째로 "INIT>" + to_cat10_cd 형식으로 결과
        line2 = f"INIT>{to_cat10_cd};{total_flow_valueB}"
    ...
    for item in data['data']:
        ...
        combined = f"{flow_cat10_id};{flow_valueB}"
